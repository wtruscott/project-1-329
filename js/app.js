...
$(".toSkills").on("click", (event) => {
    ...
    const $icons = $("<div>", {id: "icons"})
    const $resPic = $("<img>").attr("src", "https://res.cloudinary.com/dbmh779j0/image/upload/v1624799834/Screen_Shot_2021-06-27_at_9.13.14_AM_x2wl4u.png")
    const $iconSass = $("<i>").addClass("devicon-sass-original colored")
    const $iconCss = $("<i>").addClass("devicon-css3-plain-wordmark colored")
    const $iconDjango = $("<i>").addClass("devicon-django-plain-wordmark colored")
    const $iconExpress = $("<i>").addClass("devicon-express-original-wordmark colored")
    const $iconGit = $("<i>").addClass("devicon-git-plain-wordmark colored")
    const $iconGitHub = $("<i>").addClass("devicon-github-original-wordmark colored")
    const $iconHeroku = $("<i>").addClass("devicon-heroku-plain-wordmark colored")
    const $iconHTML = $("<i>").addClass("devicon-html5-plain-wordmark colored")
    const $iconJavascript = $("<i>").addClass("devicon-javascript-plain colored")
    const $iconJQuery = $("<i>").addClass("devicon-jquery-plain-wordmark colored")
    const $iconMongo = $("<i>").addClass("devicon-mongodb-plain-wordmark colored")
    const $iconNode = $("<i>").addClass("devicon-nodejs-plain-wordmark colored")
    const $iconPostgres = $("<i>").addClass("devicon-postgresql-plain-wordmark colored")
    const $iconPython = $("<i>").addClass("devicon-python-plain-wordmark colored")
    const $iconRails = $("<i>").addClass("devicon-rails-plain-wordmark colored")
    const $iconReact = $("<i>").addClass("devicon-react-original-wordmark colored")
    const $iconRuby = $("<i>").addClass("devicon-ruby-plain-wordmark colored")
    const $iconVSCode = $("<i>").addClass("devicon-vscode-plain-wordmark colored")
    $newPage.append($resume, $icons)
    $resume.append($resPic)
    $icons.append($iconSass, $iconCss, $iconDjango, $iconExpress, $iconGit, $iconGitHub, $iconHeroku, $iconHTML, $iconJavascript, $iconJQuery, $iconMongo, $iconNode, $iconPostgres, $iconPython, $iconRails, $iconReact, $iconRuby, $iconVSCode)
})
...
$(".toContact").on("click", (event) => {
    ...
    $(".container").append($newPage)
    const $contactText = $("<div>", {id: "contactText"})
    const $contactImg = $("<div>", {id: "contactImg"})
    const $name = $("<h2>").text("William Truscott")
    const $email = $("<a>", {class: "mailto", "href": "mailto:truscowa@gmail.com", "text": "truscowa@gmail.com"})
    const $phone = $("<h2>").text("(203)-770-6859")
    const $location = $("<h2>").text("Atlanta, GA")
    $contactText.append($name, $email, $phone, $location)
    const $mailPlane =$("<i>").addClass("far fa-paper-plane")
    $contactImg.append($mailPlane)
    $newPage.append($contactText, $contactImg)
})
